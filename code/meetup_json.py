        ...
        if os.path.isfile(file):        
            ...
            with open(file) as data_file:    
                ...
                for element in data['results']:
                    ...
                    created = element['created'] if 'created' in element else ''
                    time_collapsed = int(round(time.time() * 1000)) - created
                    in_readable = str(datetime.timedelta(milliseconds=time_collapsed))    
                    created = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(created/1000.0))                    
                    members = element['members'] if 'members' in element else ''
                    join_mode = element['join_mode'] if 'join_mode' in element else ''
                    rating = element['rating'] if 'rating' in element else ''
                    ...
                    group_list[element['id']] = element['name'].replace(',', '') + ", " + created + "," + description.replace(',','') + ',' + ecity.replace(',', '') + ", "      
                    group_list[element['id']] += category.replace(',', '') + ", " + str(lon) + ", " + str(lat) + "," + str(members) + "," + join_mode
                    group_list[element['id']] += ", "  + topics.replace(',', '') + "," + str(num_topics) + "," + str(rating) + "," + str(time_collapsed) + "," + in_readable
